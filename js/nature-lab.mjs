// File: /js/nature-lab.mjs
// --- "Guess the Animal" Game Logic ---
...
/**
 * loadAnimalsPool
 * @description Fetches the list of animals from a local JSON file.
 */
...
/**
 * fetchFactsViaProxy
 * @description Fetches fun facts for a given animal via a proxy endpoint.
 */
...
/**
 * formatFacts
 * @description Formats the fetched animal data into an HTML list.
 */
function formatFacts(animal) {
  if (!animal) { return ""; }
  const tx = animal.taxonomy || {}, ch = animal.characteristics || {}, locs = Array.isArray(animal.locations) ? animal.locations : [];
  const lines = [];
...
const resetGameState = () => { questions = []; currentRound = 0; score = 0; };
resetGameState();

/**
 * startQuiz
 * @description Sets up and starts a new 5-round game session.
 */
function startQuiz() {
  resetGameState();
  ...
  for (let i = 0; i < TOTAL_ROUNDS; i++) {
    const specimen = roundAnimals[i * 2], distractor = roundAnimals[i * 2 + 1];
...
/**
 * nextRound
 * @description Renders the next round or shows the summary if the game is over.
 */
function nextRound() {
...
/**
 * handleResult
 * @description Processes the result of a round, shows feedback and facts, and provides the next step.
 */
...
/**
 * onAnswer
 * @description Handles the user's answer click event.
 */
...
/**
 * timeUp
 * @description Handles the event when the round timer reaches zero.
 */
...
/**
 * showSummary
 * @description Displays the final summary and saves the game session to localStorage.
 */
function showSummary() {
  ...
  containerRef.innerHTML = `
    <div class="quiz-body" style="text-align: center;">
      <h2>Quiz Complete!</h2>
      <div class="fact-card">
        <p style="font-size: 1.5rem; margin: 1rem 0;">Your final score is: <strong>${score} / ${TOTAL_ROUNDS}</strong></p>
      </div>
      <div style="margin-top: 2rem;"><button id="play-again-btn">Play Again</button></div>
    </div>`;
...
/**
 * renderRound
 * @description Renders the UI for the current round of the game.
 */
...
/**
 * initNatureLab
 * @description Main function to initialize the "Guess the Animal" game.
 */
